...
class NotesControllerTest < ActionController::TestCase
  ...
  context "as admin" do
    setup do
      @admin_user = Factory.create(:user, :role => User::ADMIN)  
    ...
    should "be able to create a new note for student" do
       assert_difference "Note.count", 1 do
          attrs = Factory.attributes_for(:note, :student => @student, :user => @admin_user)       
    ...
    should "be able to update note within #{Note.EDIT_TIME / 60} minutes" do
      note = Factory.create(:note, :student => @student, :user => @admin_user)
    ...
    should "redirect to notes list with error mesage if updating after #{Note.EDIT_TIME / 60} minutes" do
      note = Factory.create(:note, :created_at => Time.now - 2.days, :student => @student)
      note.reload
    ...
    should "be not be able to update note after #{Note.EDIT_TIME / 60} minutes" do
      note = Factory.create(:note, :created_at => Time.now - 6.minutes, :student => @student )
    ...
    should "be able to delete the notes" do
      ...
      note = Factory.create(:note, :student => @student)
    ...
    should "list all the notes for student" do
      note = Factory.create(:note, :student => @student)
      note2 = Factory.create(:note, :student => @student)
