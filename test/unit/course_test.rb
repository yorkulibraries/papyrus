...
class CourseTest < ActiveSupport::TestCase
  ...
  should "be retrieved ordered alphabetically by default" do
    Factory.create(:course, :title => "b")
    Factory.create(:course, :title => "c")
    Factory.create(:course, :title => "a")
  ...
  should "not create a course without a term" do
    course = Factory.build(:course, :term => nil)
  ...
  should "not create/update a course without title or code" do
    course = Factory.build(:course, :title => nil)
    assert !course.valid?
    
    course2 = Factory.build(:course, :code => nil)
  ...
  should "not create a course with a duplicate code" do
    ...
    course_dup = Factory.build(:course, :code => code)
  ...
  should "search for courses and ignore courses with expired Terms" do
    active_term = Factory.create(:term, :name => "active", :start_date => Date.today, :end_date => Date.today + 1.year)
    expired_term = Factory.create(:term, :name => "expired", :start_date => Date.today - 1.year, :end_date => Date.today - 2.months)
    
    Factory.create(:course, :title => "Expired same_string", :term => expired_term, :code => "1")
    Factory.create(:course, :title => "Active same_string", :term => active_term, :code => "2")
