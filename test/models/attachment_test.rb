...
class AttachmentTest < ActiveSupport::TestCase
  should 'create a new valid attachment' do
    attachment = build(:attachment, name: 'woot', full_text: true,
  ...
  should 'list all available attachments and deleted attachments' do
    available = create_list(:attachment, 2, file: fixture_file_upload('../test_pdf.pdf', 'application/pdf'))
  ...
  should 'show full_text, and non_full text (available) attachments' do
    full_text = create_list(:attachment, 2, file: fixture_file_upload('../test_pdf.pdf', 'application/pdf'),
                                            full_text: true)
  ...
  should 'name the file properly and store it in the proper place' do
    ...
    attachment = create(:attachment, file: fixture_file_upload('../test_pdf.pdf', 'application/pdf'), item:)

    assert attachment.file, 'File was saved'
    assert attachment.file.filename.end_with?('-test_pdf.pdf'),
           'File name should include the timestamp and end with file name'
    assert_equal "/uploads/items/book/#{item.unique_id}/#{attachment.file.filename}", attachment.file.to_s,
