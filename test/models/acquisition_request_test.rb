...
class AcquisitionRequestTest < ActiveSupport::TestCase
  ...
  should "require additional acquired fields if Status is #{AcquisitionRequest::STATUS_ACQUIRED}" do
    ...
    assert !build(:acquisition_request, status:, acquired_by: nil).valid?
    assert !build(:acquisition_request, status:, acquired_at: nil).valid?
    assert !build(:acquisition_request, status:, acquisition_source_type: nil).valid?
    assert !build(:acquisition_request, status:, acquisition_source_name: nil).valid?
  ...
  should "require additional cancelled fields if Status i #{AcquisitionRequest::STATUS_CANCELLED}" do
    ...
    assert !build(:acquisition_request, status:, cancelled_by: nil).valid?
    assert !build(:acquisition_request, status:, cancelled_at: nil).valid?
    assert !build(:acquisition_request, status:, cancellation_reason: nil).valid?
