...
class Item < ApplicationRecord
  ...
  def assign_to_student(student, expire_on = nil)
    existing_connection = ItemConnection.where(
      'student_id = ? AND item_id = ? AND (expires_on >= ? OR expires_on IS ?)', student.id, id, Date.today, nil
    ).first
    ...
    if existing_connection
      ...
      connection.audit_comment = "Assigned - #{title[0...240]}"
      connection.save
  ...
  def withhold_from_student(student)
    ...
    connection = ItemConnection.where('student_id = ? AND item_id = ? AND (expires_on >= ? OR expires_on IS ?)',
                                      student.id, id, Date.today, nil).first

    unless connection.nil?
      connection.audit_comment = "Removed #{title}"
      connection.destroy
    end
  end
