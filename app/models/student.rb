class Student < User
  ...
  has_one :student_details_only_student_number, lambda {
                                                  select 'student_details.student_number'
                                                }, class_name: 'StudentDetails', foreign_key: 'student_id'
  ...
  has_many :current_items, lambda {
                             where('item_connections.expires_on >= ? OR item_connections.expires_on IS ?', Date.today, nil)
                           }, through: :item_connections, source: :item
  has_many :expired_items, lambda {
                             where('item_connections.expires_on < ?', Date.today)
                           }, through: :item_connections, source: :item
  ...
  scope :most_recent_students, ->(lim) { order(id: :desc).limit(lim) }

  scope :never_logged_in, -> { where('last_logged_in_at is NULL') }

  def to_csv
    [id, name, email, student_details.student_number, student_details.formats.join(', '),
     student_details.cds_counsellor, created_at]
  end
  ...
  def formats_array
    formats_array = []
    formats_array.push 'PDF' if details.format_pdf
    formats_array.push 'KURZWEIL' if details.format_kurzweil
    formats_array.push 'DAISY' if details.format_daisy
    formats_array.push 'BRAILLE' if details.format_braille
    formats_array.push 'WORD' if details.format_word
    formats_array.push 'LARGE PRINT' if details.format_large_print
    formats_array.push 'EPUB' if details.format_epub
    formats_array.push 'OTHER' if details.format_other
    formats_array
  end
  ...
  def self.item_counts(student_ids, type = 'current')
    ...
    if student_ids.is_a?(Array) && student_ids.size < 100
      connections = connections.where('student_id in(?)',
                                      student_ids)
    end
  ...
  def self.build_hash_from_array(data, fields)
    ...
    fields.each_with_index do |field, index|
      ...
      when 'accessibility_lab_access'
        hash[:student_details_attributes][:accessibility_lab_access] =
          data[index].try(:downcase) == 'true'
      when 'book_retrieval'
        hash[:student_details_attributes][:book_retrieval] = data[index].try(:downcase) == 'true'
      when 'alternate_format_required'
        hash[:student_details_attributes][:alternate_format_required] =
          data[index].try(:downcase) == 'true'
      when 'format_pdf'
        hash[:student_details_attributes][:format_pdf] = data[index].try(:downcase) == 'true'
      when 'format_large_print'
        hash[:student_details_attributes][:format_large_print] = data[index].try(:downcase) == 'true'
      when 'format_daisy'
        hash[:student_details_attributes][:format_daisy] = data[index].try(:downcase) == 'true'
      when 'format_braille'
        hash[:student_details_attributes][:format_braille]  = data[index].try(:downcase) == 'true'
