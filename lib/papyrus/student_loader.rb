...
module Papyrus
  class StudentLoader
    ...
    def from_list(students_list, options = nil)
      return nil if students_list.nil? || !students_list.is_a?(Array) # If nil or array return
      ...
      students_list.each do |student_array|
        ...
        student = Student.where('student_details.student_number = ?',
                                params[:student_details_attributes][:student_number]).references(:student_details).includes(:student_details).first
        ...
        if student
          ...
          status[:errors].push [student_array, student.errors.messages] unless student.valid?
          ...
          if student.save
            ...
            if PapyrusSettings.import_notify_coordinator == PapyrusSettings::TRUE && student.details.transcription_coordinator
              ...
              ReportMailer.mail_report(student.details.transcription_coordinator.email, body,
                                       'New Student Assigned').deliver_later
            end
