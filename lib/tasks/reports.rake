...
namespace :reports do
  ...
  task :item_usage => :environment do 
    ...
    @export_items  = @unassigned_items = Item.where("id not in (?)", @assigned_items.collect { |i| i["id"] }) if ENV["UNASSIGNED"]
    ...
    csv_file = CSV.generate(options) do |csv|
      ...
      @export_items.each do |item|       
        ...
        csv << [item["title"].to_s, item["callnumber"].to_s, item["source"].to_s, item["isbn"].to_s, item["assigned_count"].to_s]
      end                  
    end
