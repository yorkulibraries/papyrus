Rails.application.routes.draw do
  ...
  namespace :students do
    ...
    resource :lab_access_only, only: %i[show destroy], controller: 'lab_access_only'
  ...
  scope '/find' do
    resources :local_items, only: %i[new index], module: 'search', as: 'find_items'
    resource :omni, only: %i[new show], module: 'search', controller: 'omni', as: 'omni_search'
  end
  ...
  resources :announcements do
    get 'hide', on: :member
    resource :activation, only: %i[update destroy], controller: 'announcements/activation'
  end
  ...
  resources :shared_access_codes, only: %i[index new create destroy]

  namespace :students do
    resources :permanent_delete, only: %i[index show destroy]
  end

  resources :students do
    resources :documents
    resources :access_codes
    resources :notes, except: %i[show new]

    resource :student_details, as: :details, path: 'details', except: %i[index destroy]

    resource :bulk_renew_items, only: %i[new create show], module: 'students'
  ...
  resources :items do
    ...
    resources :attachments, path: 'files', except: %i[index show] do
    ...
    collection do
      ...
      resources :from_primo, only: %i[index new create], module: :items
      resources :from_worldcat, only: %i[index new create], module: :items
    end
  end
  ...
  match 'search/items(/:type)' => 'search#items', as: 'search_items', via: %i[get post] # ,  defaults: { type: "local" }
